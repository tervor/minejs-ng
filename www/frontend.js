...
$(document).ready(function() {
    ...
    //   onmouseover="boxHover(\''+id+'\');" onmouseout="$(this)(\''+id+'\')"
        $('#sortable').append(' \
            <div id="box-'+id+'" class="GridBox ItemGridBox"> \
            <div id="IncDecMinusButton-'+id+'" class="incdecbuttonbox" onclick="IncDecMinus(\'IncDecInput-'+id+')">-</div> \
            <div id="IncDecInput-'+id+'" value="'+amount+'" class="incdecinputbox"  \
            DISABLED  onclick="giveItemById(\''+id+'\',\''+amount+'\',\'IncDecInput-'+amount+'\')"> \
            <div id="IncDecPlusButton-'+id+'" class="incdecbuttonbox" onclick="IncDecPlus(\''+id+'\')">+</div> \
            <div name="IconLayer-'+id+'" style="height:100%;width:100%;"  onclick="giveItemById(\''+id+'\',\''+amount+'\',\'IncDecInput-'+id+'\')" > \
            <div style="padding: 25% 29% 4% 29%;"> \
            <img src="icons/'+id+'.pn" id=="ItemImg-'+id+'" class="STicon" border="0"></div> \
            <div name="Namelabel-'+id+'"  id="Namelabel-'+id+'" class="Namelabel">'+name+'</div></div></div> ');
